...
agent 简报是 issue 或 PR 移到 `ready-for-agent` 时贴在 GitHub issue 或 PR 上的结构化评论。它是离席 agent 将依据的权威规范。原始正文和讨论是上下文——agent 简报才是契约。

简报陈述**agent 应该做什么**，这延伸到两个面：对 issue，是从零构建变更；对 PR，是对*现有 diff* 剩下要做的——完成它、补缺口、处理审查意见。两种形态原则相同；下面的 PR 示例展示了差异。

## 原则

### 持久性优先于精确性

issue 可能在 `ready-for-agent` 里待几天或几周。代码库在此期间会变。把简报写得即使文件被重命名、移动或重构也仍然有用。
...
描述系统**应该做什么**，而不是**怎么实现**。agent 会全新探索代码库并自己做实现决策。
...
agent 需要知道何时算做完。每个 agent 简报必须有具体、可测试的验收标准。每条标准应可独立验证。
...
陈述什么超出范围。这防止 agent 镀金或对相邻功能做假设。
...
对 PR，「Current behavior」描述 diff 的状态，简报请 agent 完成或修复它，而不是从零构建。
